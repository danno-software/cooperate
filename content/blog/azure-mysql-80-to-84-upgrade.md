...
MySQL 8.0 の Extended Support 期限が 2026年4月に迫っています。Azure Database for MySQL Flexible Server を利用している場合、8.4 へのアップグレードを検討している方も多いのではないでしょうか。
...
なぜこの順番かというと、Azure のマネージドサービスは**アップグレード時にある程度のことを自動で吸収してくれる**からです。事前に手を動かす前に、「Azure がどこまでやってくれるのか」を先に把握した方が効率的です。
...
**アップグレードに必須の事前対応はゼロ**でした。Azure のマネージドサービスが、互換性に関わる変更を全て自動で吸収してくれました。

もちろん、これは今回の環境での結果です。利用しているMySQLの機能やアプリケーションの構成によっては、事前対応が必要になるケースもあります。

ただ、「まず上げてみて差分を確認する」というアプローチを取ることで、**本当に対応が必要な項目だけに集中**でき、検証を効率的に進めることができました。

## まとめ

- MySQL 8.0 の Extended Support 期限が迫っている。早めの検証開始がおすすめ
- 検証は「事前に全部潰す」よりも、**「まず上げてみて差分を確認する」** 方が効率的
- AIを活用すれば、網羅的なチェックスクリプトを短期間で作成できる。**Claude Code で作り、Codex でレビュー**するクロスレビューが有効
- Azure のマネージドサービスは、多くの互換性の問題を自動で吸収してくれる
